...
#build fuzzer for DUML packet
def fuzz_duml_packet():
    """ Fuzzes DUML packet with random values.
    """
    pkthead = DJICmdV1Header()
    pkthead.whole_length = random.randint(0, 255) #fix minimal size
    pkthead.header_crc8 = calc_pkt55_hdr_checksum(0x77, (c_ubyte * 3).from_buffer_copy(pkthead), 3)
    pkthead.sender_type = random.randint(0, 31)
    pkthead.sender_index = random.randint(0, 7)
    pkthead.receiver_type = random.randint(0, 31)
    pkthead.receiver_index = random.randint(0, 7)
    pkthead.seq_num = random.randint(0, 511)
    pkthead.packet_type = random.randint(0, 7)
    pkthead.ack_type = random.randint(0, 3)
    pkthead.encrypt_type = random.randint(0, 7)
    pkthead.cmd_set = random.randint(0, 255)
    pkthead.cmd_id = random.randint(0, 255)
    enc_data = (c_ubyte * pkthead.whole_length)()
    memmove(addressof(enc_data), byref(pkthead), sizeof(pkthead))
    pktpayload = (c_ubyte *(pkthead.whole_length - sizeof(pkthead)))()
    for i in range(0, len(pktpayload)):
        pktpayload[i] = random.randint(0, 255)
    memmove(addressof(enc_data) + sizeof(pkthead), byref(pktpayload), sizeof(pktpayload))
    pktfoot = DJICmdV1Footer()
    pktfoot.crc16 = calc_pkt55_checksum(enc_data, sizeof(enc_data) - 2)
    memmove(addressof(enc_data) + sizeof(pkthead) + sizeof(pktpayload), byref(pktfoot), sizeof(pktfoot))
    print(' '.join('{:02x}'.format(x) for x in enc_data))
